...
function articleMd(opts: { title: string; slug: string; category: string; tags?: string[]; summary?: string; body?: string }) {
...
describe("generateIndexMd", () => {
	test("generates index with articles grouped by category", async () => {
		...
		await writeWiki(root, "concepts/transformers.md", articleMd({
		...
		await writeWiki(root, "topics/scaling-laws.md", articleMd({
			title: "Scaling Laws",
			slug: "scaling-laws",
			category: "topic",
			tags: ["training"],
		...
		await writeWiki(root, "references/vaswani.md", articleMd({
			title: "Vaswani et al.",
			slug: "vaswani-et-al",
			category: "reference",
	...
	test("sorts articles alphabetically within categories", async () => {
		...
		await writeWiki(root, "concepts/zebra.md", articleMd({
			title: "Zebra Concept", slug: "zebra", category: "concept",
		}));
		await writeWiki(root, "concepts/alpha.md", articleMd({
			title: "Alpha Concept", slug: "alpha", category: "concept",
		}));
		await writeWiki(root, "concepts/mid.md", articleMd({
			title: "Mid Concept", slug: "mid", category: "concept",
		}));
	...
	test("skips INDEX.md and GRAPH.md from listing", async () => {
		...
		await writeWiki(root, "concepts/test.md", articleMd({
			title: "Test", slug: "test", category: "concept",
		}));
...
describe("computeStats", () => {
	test("computes article count and word count", async () => {
		...
		await writeWiki(root, "concepts/a.md", articleMd({
			title: "A", slug: "a", category: "concept",
			body: "one two three four five",
		}));
		await writeWiki(root, "concepts/b.md", articleMd({
			title: "B", slug: "b", category: "concept",
